...
// 🟢 ফায়ারবেস ফ্রি রিড লিমিট বাঁচানোর জন্য ব্যাকএন্ড ইন-মেমোরি কাস্টমার ক্যাশ
let customerCacheMap = null;
let lastCacheTime = 0;

// ক্যাশ ক্লিয়ার করার ফাংশন
function invalidateCustomerCache() {
  customerCacheMap = null;
  lastCacheTime = 0;
  console.log("Customer cache reset.");
}

async function getCachedCustomers(db) {
  const now = Date.now();
  // ১২ ঘণ্টা পর পর অথবা ক্যাশ খালি থাকলে ফায়ারবেস থেকে ১ বার লোড হবে (৯৯.৩% ফায়ারবেস রিড সেভ করবে)
  if (!customerCacheMap || (now - lastCacheTime > 12 * 60 * 60 * 1000)) {
    console.log("Fetching customers from Firestore and caching in backend memory...");
    ...
    snapshot.docs.forEach(doc => {
      ...
      if (pppoe) newMap.set(pppoe, data);
    });

    customerCacheMap = newMap;
    lastCacheTime = now;
  }
  return customerCacheMap;
}

// 🟢 মাইক্রোটিক রাউটার থেকে লাইভ অন-লাইন পিপিইওই কাস্টমার রিড
...
async function runNetworkDiagnostics() {
  try {
    ...
    // 🟢 ২. ফায়ারবেসের ফ্রি লিমিট বাঁচাতে ক্যাশ মেমোরি থেকে কাস্টমার লিস্ট রিড (০ ফায়ারবেস রিড খরচ)
    const allCustomersMap = await getCachedCustomers(db);
    if (allCustomersMap.size === 0) return [];
    ...
    // ৪. ফায়ারবেস থেকে আগের স্ক্যানের স্থায়ী অনলাইন স্টেট লোড
    ...
    if (!stateSnap.exists) {
      ...
      console.log(`Initial Baseline Set: Online ${totalOnlineCount}`);
      return [];
    }
    ...
    // 🔴 ৬. ডিসকানেক্ট হওয়া কাস্টমার বের করা
    ...
    // 🔔 ৮. লাইন অফলাইন / লোডশেডিং নোটিফিকেশন
